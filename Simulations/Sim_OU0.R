...
Rcpp::sourceCpp("MSCR/LikelihoodC.cpp")
...
traps = make.grid(sqrt(S), sqrt(S) , spacex = 1, detector = "multi") # start by creating a grid as if each cell was a trap
mesh = make.mask(traps,buffer=0.5,type="traprect",spacing=mesh_spacing) # mesh encompassing area around traps
mask <- mesh
...
sigma_sq <- log(1) # natural scale 


# Attraction to activity centre
#beta <- 1e-6 # natural scale

beta <- 1e-6


#theta <- c(log(sigma_sq), log(beta)) 
...
Q <- neighbour * exp(sigma_sq) / (2 * mesh_spacing^2) 
...
for (j in 1:n_sim) {
 ...
 # s0<-pre_data[[3]]
  ...
  results[j, "alpha_hat"] <- param_umove[3, 1]
  results[j, "se_lambda"] <- param_umove[4, 2]
  results[j, "se_alpha"] <- param_umove[3, 2]
