################################################################################
# Simulation study from the OUMove-SCR model with parameter alpha = 0
...
grid = make.grid(sqrt(S), sqrt(S) , spacex = 1, detector = "multi") 
mesh = make.mask(grid,buffer=0.5,type="traprect",spacing=mesh_spacing)
a <-  attr(mesh, "a")
...
# Create a finer grid covering the same area to draw true activity centre locations from 
fine_spacing <- 0.1 
...
# Define the  coarser integration mesh, here equal to the state space mesh 
coarse_spacing <- 1.0 
...
# Detection rate parameter lambda
l <- 0.5 
...
Q <- neighbour * sigma_sq / (2 * mesh_spacing^2) 
...
for (j in 1:n_sim) {
  ...
  Q_fixed <- 1 - neighbour         # Positions of structural zeros (sigma^2 is the first parameter)
  ...
  lambda_template[traps_on] <- 2 # lambda is the second parameter
  ...
  ################ FIT WITH OUMove-SCR MODEL #####################
  ################################################################
 
  cat("\n--- Fitting OUMove-SCR model ---\n")
  ...
  # Initial parameters: log(sigma^2), log(alpha), log(lambda)
  ...
  # UMOVE results
  results[j, "lambda_hat"] <- param_umove[4, 1]
  results[j, "sigmasq_hat"] <- param_umove[3, 1]
  results[j, "se_lambda"] <- param_umove[4, 2]
  results[j, "se_sigmasq"] <- param_umove[3, 2]
...
# Save results to CSV
write.csv(results, "simulation_results_0.csv", row.names = FALSE)
