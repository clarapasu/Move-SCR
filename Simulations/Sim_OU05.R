...
for (j in 1:n_sim) {
  ...
  traps_on<-sort(sample(1:S,size=n_traps))
  trap <- mesh[traps_on,]
 ...
 # plot_trajectory(obj, 4, mesh, trap)
  ...
  fit_nomem <- optim(theta_init[1:2], LikelihoodCnoMem, trap = traps ,
                     df = ddfmat_sim, dfrows =dfrows_sim, mesh = as.matrix(mesh_ac_fit), endt = Time, hessian=TRUE)
...
# Save results to CSV
write.csv(results, "simulation_results_OU05_bis.csv", row.names = FALSE)
